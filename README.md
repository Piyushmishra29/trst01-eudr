...
**Loading screen and the stale-file freeze.** The page once froze on its loading screen: the browser paired a fresh `index.html` with a cached older `models.js`, the import failed, nothing said so. Now `deploy.sh` stamps `BUILD` into the page and every script, JSON and image is asked for with `?v=BUILD`; the server sends `no-cache` for the page itself; any error before the scene is up shows on the loader and reloads once by itself, then offers "Try again". The small-objects layer (1.6 MB + build time) now loads after the scene is on screen, in 10 ms slices. The loader is the survey being flown: a drone sweeps a lawnmower pattern over the real parcel plan (from `site.json`), boundaries trace in and acres count up with real load progress, then the plan zooms away into the 3D glide-in. `?loaderdemo` keeps it up to look at. Always publish with `./deploy.sh` (a plain rsync skips the stamp).
