...
for t, (cx, cy, rx, ry, ht, palm) in enumerate(T):
...
for b in B:
    poly = np.array(b["poly"], float); h = (0.25 + 0.6 * b["h"]) * 10; e = en(poly[:, 0], poly[:, 1]); w = np.c_[e[:, 0], -e[:, 1]]; n = len(poly); ctr = np.array([w[:, 0].mean(), h / 2, w[:, 1].mean()])
    for i in range(n):                              # walls, one quad per edge (own vertices: crisp corners)
        j = (i + 1) % n; add_col(np.array([[w[i, 0], h, w[i, 1]], [w[j, 0], h, w[j, 1]], [w[i, 0], -0.5, w[i, 1]], [w[j, 0], -0.5, w[j, 1]]]), WALL, outward(np.array([[w[i, 0], h, w[i, 1]], [w[j, 0], h, w[j, 1]], [w[i, 0], -0.5, w[i, 1]], [w[j, 0], -0.5, w[j, 1]]]), [[0, 1, 2], [2, 1, 3]], ctr))
    if b["roof"] == "gable":
